...
class ModerateMessage:
    async def moderate_message(self, misinformationCategories):
        message = allConnectedReasoning()
        ...
        db.addText(f"Contains violation: {contains_violation}, violated categories: {violated_categories}, explanation: {explanation}")
        return {"violations" : f"Contains violation: {contains_violation}, violated categories: {violated_categories}, explanation: {explanation}"}
...
class RecursiveRetrievalModule:
    ...
    async def iterative_retrieve(self: str) -> List[Dict]:  #max_steps
        ...
        if queries != []:
            for ind in range(3):
                query = "<DataSeparator>".join(queries)
...
class GenerateInsights:
    async def process_documents(self) -> Dict:
        ...
        llm = LLM(system="Your aim is to tell whether this data is factually correct and valid or not. Identify key insights, detect potential factual inconsistencies, but keep in mind satire, humour and other non-objective statements, however make sure to explain your reasoning.")
...
class RecursiveGenerationModule:
    async def generate_final_answer(self, overall_insights, initial_query: str) -> str:
        ...
        prompt = f"""Evaluate the reliability of the user-provided data based on the following insights: {overall_insights['insights']}. Remember, your goal is to fact check, so be straight to the point, without waffle, yet explain your reasoning in clear steps, be insightful but ration your output. 
...
async def run_recursive_pipeline(initial_query):
    ...
    moderation_module = ModerateMessage()
    ...
    # moderated_message = moderation_module.moderate_message(getMisinformationCategories())
    # print("Moderator Review:", moderated_message)

    print("\n[STEP 2] Message Moderation...")
    moderated_message = await moderation_module.moderate_message(getMisinformationCategories())
    print("Moderator Review:", moderated_message)

    # print("\n[STEP 3] Processing Documents...")
    # insights = await generate_insights.process_documents()
    # print("Reasoning Output:", insights["violations"])

    print("\n[STEP 3] Processing Documents...")
    insights = await generate_insights.process_documents()
    print("Reasoning Output:", insights["insights"])  # Use the correct key
...
def send_prompt(query, links, files):
    ...
    for line in query.splitlines():
        db.addText(f"THIS ONE OF THE QUERIES, TREAT IT AMONG THE MOST IMPORTANT SOURCES OF INFORMATION AND THE THREAD OF THE CONVERSATION, MAKE SURE TO ADDRESS IT AT THE FINAL ANSWER: {line}")
    for file in files[:1]:
...
#     print(send_prompt(query, links, files))
